...
import { buildOpenMeteoHourlyForecast, parseHourLabel } from './weather'
...
describe('Open-Meteo hourly forecasts', () => {
  test('keeps offsetless local forecast hours from API order', () => {
    ...
    expect(hourly).toEqual([
      { hour: '23:00', hourNum: 23, temperature: 18, weatherCode: 0, precipitationProbability: 0 },
      { hour: '0:00', hourNum: 0, temperature: 18, weatherCode: 1, precipitationProbability: 10 },
      { hour: '1:00', hourNum: 1, temperature: 18, weatherCode: 2, precipitationProbability: 20 },
    ])
  })
